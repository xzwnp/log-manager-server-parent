	...
	public void importApps() {
		...
		for (Map.Entry<String, Set<String>> appGroup : appGroupMap.entrySet()) {
			...
			if (sysAppPo == null) {
				...
				appPos.add(sysAppPo);
				log.info("app {} not exist,auto creating app ...", appName);
			} else {
				log.info("app {} has existed,updating ...", appName);
			}
			sysAppPo.setGroups(groups);
		}
		//存入数据库
		sysAppService.saveOrUpdateBatch(appPos);
		log.info("save app info success");
		...
		log.info("auto grant appAdmin role of {} apps for {} success", appIds.size(), UserThreadLocalUtil.getUserName());
	}
